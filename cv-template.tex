...
$for(paper)$
$if(paper.pub)$
$paper.author$ ($paper.pub.year$), ``$paper.title$.'' $paper.pub.journal$. $paper.pub.volume$: $paper.pub.pp$.
\medskip\bp
...
\begin{samepage}
$if(paper.wp)$
$paper.author$, ``$paper.title$'' $if(paper.status)$\textit{$paper.status$}$endif$
